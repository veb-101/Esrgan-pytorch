...
class DenseBlock(nn.Module):
    def __init__(self, nf=64, gc=32, res_scale=0.2):
        ...
        # 64+ 32  + 32-> 32
        self.conv3 = spectral_norm(nn.Conv2d(nf + 2 * gc, gc, 3, 1, 1))
        # 64+ 32  + 32+ 32 -> 32
...
class Generator(nn.Module):
    def __init__(self, channels=3, nf=64, gc=32, num_res_blocks=16, scale=4):
        ...
        #### upsampling
        self.upconv1 = spectral_norm(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.upconv2 = spectral_norm(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
        self.HRconv = spectral_norm(nn.Conv2d(nf, nf, 3, 1, 1, bias=True))
    ...
    def forward(self, x):
        fea = self.conv1(x)
        trunk = self.conv2(self.res_blocks(fea))
        fea = fea + trunk
